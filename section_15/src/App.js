...
export default class App extends React.Component {
  ...
  async updateTranslatedMovies() {
    ...
    const movies = await Promise.all(this.state.rawMovies.map(async movie => {
      return {
        id: movie.id,
  ...
  handleLanguageChange(event) {
    this.setState({ language: event.target.value }, this.updateTranslatedMovies);
  }
  ...
  render() {
    return (
      <React.Fragment>
        ...
        <section>
          {this.state.isLoading ? <p>Loading movie data...</p> : <MoviesList movies={this.state.movies} />}
          {!this.state.isLoading && this.state.movies.length === 0 && <p>Try loading some movies.</p>}
        </section>
      </React.Fragment>
    );
  }
